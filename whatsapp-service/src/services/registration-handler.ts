...
import { API_TOKEN, FE_URL } from "../config";
 ...
 */
async function isVoterRegistered(phone: string) {
	const req = await fetch(`${FE_URL}/api/voters/find?phone=${phone}`, {
		headers: {
			Authorization: `Bearer ${API_TOKEN}`,
		},
	});

	const res = (await req.json()) as Record<string, unknown>;
 ...
 */
async function saveVoter(phone: string, name: string, organization: string) {
	const req = await fetch(`${FE_URL}/api/voters/create`, {
		method: "POST",
		headers: {
...
export async function handleRegistration(
	...
	console.log(`🔍 Voter already registered: `, alreadyRegistered);
	...
	console.log(`💾 Save voter response: `, saved);
